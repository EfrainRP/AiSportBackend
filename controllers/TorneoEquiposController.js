...
import {brackets} from "../utils/formatBracket.js";
 ...
 export const index = async (req, res) => {
    ...
    const matchIdBracket = parseInt(req.query.matchIdBracket);
    ...
    try {
            ...
            select: {
                ...
                partidos: {
                    include: {
                        ...
                        equipos_partidos_equipoVisitante_idToequipos: true, 
                    },
                    orderBy: {
                        ordenPartido: 'asc',
                    },
        ...
        const teamsWinners = theBracket.filter(bracket => matchIdBracket === bracket.nextMatchId);
        console.log('id ',  matchIdBracket);
        console.log('teamsWinners ',  teamsWinners);
        if(teamsWinners.length){
            const participants = teamsWinners.map((bracket) => {
                const team1 = bracket.participants[0];
                const team2 = bracket.participants[1];

                if(team1.isWinner === team2.isWinner){
                    const random = Math.floor(Math.random()*2);  //random de entre 0 y 1
                    const teamWin = random ? team1 : team2;
                    return {'id': teamWin.id, 'name':teamWin.name}
                }   
                return team1.isWinner ? {'id': team1.id, 'name':team1.name} : {'id': team2.id, 'name':team2.name};
            });
            console.log(participants);
            res.status(200).json(participants);
        }else{
            // Obtener equipos que aún no han jugado en el torneo
            const equiposRestantes = await prisma.equipos.findMany({
                where: {
                    NOT: {
                        estadisticas: {
                            some: {
